    ...
    tabla = $('#tabla-datos').DataTable({
        ...
        data: filteredData.map(d => {
            ...
            return [
                ...
                `<div class='oferta-format oferta-truncada' data-fulltext="${d.Descripción}">${d.Descripción.substring(0, 950).replace(/\n/g, '<br>')} </div>`,
                `<div class='oferta-format oferta-truncada' data-fulltext="${d['Ámbito de aplicación']}">${d['Ámbito de aplicación'].substring(0, 500).replace(/\n/g, '<br>')} </div>`
            ];
        })
    });
